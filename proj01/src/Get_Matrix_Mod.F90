...
contains
    ...
    subroutine Get_Matrix_1D_Order4
        ...
        n = A%n
        if (n <= 4) stop "too few meshes to apply 4th order scheme"
        if (.not. allocated(A%row)) stop "A%row not allocated"
    ...
    subroutine Get_Matrix_2D_Order2
        ...
        ! first block (each block consists of n rows)
        do i = 1, n
            indices_1 = i
    ...
    subroutine Get_Matrix_2D_Order4
        ! output: A for 2D problem with 4th order finite difference

        use Solve_Common_Mod, only : A
        implicit none
        integer, dimension(1) :: indices_1 ! one-term index
        integer, dimension(5) :: indices_5 ! five-term indices
        integer, dimension(9) :: indices_9 ! nine-term indices
        real, dimension(1) :: values_1 = [1.0]
        real, dimension(5) :: values_5 = [1.0, 1.0, -4.0, 1.0, 1.0]
        real, dimension(9) :: values_9 = [-1.0/12.0, 4.0/3.0, -1.0/12.0, &
                      4.0/3.0, -5.0, 4.0/3.0, -1.0/12.0, 4.0/3.0, -1.0/12.0]
        ...
        ! 2nd block & (n-1)th block
        do i_block = 2, n-1, n-3
        ...
        do i_block = 3, n-2
            ...
            ! 3rd to (n-2)th line in the block
            do i = 3, n-2
                i_row = n*(i_block-1) + i
                indices_9 = [i_row-2*n, i_row-n, i_row-2, i_row-1, i_row, &
                                        i_row+1, i_row+2, i_row+n, i_row+2*n]
                call A%row(i_row)%Init(9, indices_9, values_9)
            end do
